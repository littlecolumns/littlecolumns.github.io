...
var $module = function ($B) {
    ...
    function makeTagDict(tagName) {
        ...
        dict.__init__ = function () {
            var $ns = $B.args('__init__', 1, { self: null }, ['self'], arguments, {}, 'args', 'kw'),
                self = $ns['self'],
                args = $ns['args'];
            if (args.length == 1) {
                var first = args[0];
                if (isinstance(first, [str, int, float])) {
                    ...
                    // argument is another DOMNode instance
                    try {
                        self.elt.appendChild(first.elt);
                    } catch (err) {
                        throw ValueError.$factory('wrong element ' + first);
                    }
                }
            }
            ...
            for (var i = 0, len = items.length; i < len; i++) {
                ...
                } else if (arg.toLowerCase().indexOf("href") !== -1) {
                    // xlink:href
    ...
    // SVG
    var $svg_tags = ['a', 'altGlyph', 'altGlyphDef', 'altGlyphItem', 'animate', 'animateColor', 'animateMotion', 'animateTransform', 'circle', 'clipPath', 'color_profile', // instead of color-profile
    'cursor', 'defs', 'desc', 'ellipse', 'feBlend', 'foreignObject', //patch to enable foreign objects
    'g', 'image', 'line', 'linearGradient', 'marker', 'mask', 'path', 'pattern', 'polygon', 'polyline', 'radialGradient', 'rect', 'set', 'stop', 'svg', 'text', 'tref', 'tspan', 'use'];
